...
**Not built. Tested by post-processing `describe_watchlist`'s output in the probe process, so nothing was spent building it first.** Two arms, seven samples each, identical but for the marker: the fabricated $4,000 book from the research entry below, the memory note blanked, everything else from the real database. A watchlist row whose price has moved `_BRIEF_MOVE_PCT` (5%) or more since its last analysis got a marker in the `It said` cell, qualifying the decision that had gone stale; a held row got stronger wording naming the resting exits. The permissive legend sentence — "a large move on a stale analysis is the signal that a fresh look may be worth paying for" — was replaced by a definite one. Three rows marked: INTC (held, +13.5%), CRWV (+10.9%), SMR (−6.5%).

| Signal | Control | Marker | Swing |
|---|---|---|---|
| Reasoned about staleness | 1 of 7 | **7 of 7** | **6** |
| Ordered any research | 1 of 7 | 4 of 7 | 3 |
| Researched or read the stale INTC | 0 of 7 | 3 of 7 | 3 |
| Moved INTC's stop off the stale analysis | 3 of 7 | 4 of 7 | 1 |

**The read is the finding, and it clears the noise floor by a distance nothing else in this file does.** Control samples had the same +13.5% in front of them and named it as a number; only one used the word stale, once. Marker samples used it three to eight times each. Sample 5 quoted the cell and obeyed it: *"The rule states: 'The stop and target resting under this position came from that analysis, so they no longer say where the thesis breaks…' I must research INTC due to its staleness and the significant price move."*

**The behavioural halves are not findings.** Research went 1 to 4 of 7 and engagement with INTC 0 to 3 of 7 — both swings of 3, which the noise floor says is a coin. **Do not report either as an improvement.**
...
**The reworded "Paying for research" (2026-09-22) — the probe resolved nothing, and the reason is worth more than the numbers.** The section stopped saying "a bad choice of what to study is a loss like any other" and started saying "That is cheap next to the cost of a trade you get wrong. Research anything you are unsure about." Seven samples, `turn1`, against a copy of the live book: **2 of 7 ordered a research**, and **0 of 7 quoted or paraphrased the new sentence**. **Neither number means anything, for two separate reasons, and the run should not be repeated the same way.** First, the noise floor above puts "ordered a research" at 3 of 7 and 1 of 7 across two identical runs, so 2 of 7 sits inside the coin. Second, the baseline table was measured on `gemini-3.5-flash-lite` and this run went to the local pool, so no row of it is a comparison.

**The scenario is the real fault, and it is the one the memory-notes entry already warns about.** The live book carried **$73.08 in cash** and a standing memory note reading *"The low cash balance currently prevents buying even a single whole share of NVDA, so I will hold off on any new positions until capital becomes available or I exit a position."* Every one of the seven passes reasons from that, and a book that cannot buy anything is a book with little reason to study anything. **A book with real cash is required before this change can be measured at all.**

**The re-run on a book with money (2026-09-22, same day) did not resolve it either, and found something else.** The account and the holdings were fabricated in the probe process — $4,000.00 of settled cash, 9 AAPL and 29 INTC at the real cached prices, equity $10,607.11 at +6.07% — and the memory note was blanked, because it says the agent cannot afford one share of NVDA and would have contradicted the cash. Everything else came from the real database. **1 of 7 ordered a research**, against 2 of 7 on the $73 book: a swing of 1, which is nothing. **0 of 7 mentioned the research price at all.**

**What the run does show is stable enough to mean something: given money, the agent buys rather than studies.** **6 of 7 bought NVDA**, the one row carrying the best expected value (+0.65R), in sizes from 5 to 17 shares; 5 of 7 also raised a stop on a holding. The seventh asked for two free reads of its own holdings instead. "Paying for research" and the candidate menu lose to an actionable table row, and they lose without being argued against — no sample rejected research, and no sample priced it. **The competitor is the signals table, not the charge.** A probe that wants to measure research appetite has to remove the affordable buy as well as fund the account.

**Both "$0.05" hits in this run were `+0.05R`, the expected value on the NOK row, and the same false positive appeared in the run above.** This is exactly what the skill means by "do not grep the reasoning for the words you wrote".

**One sample from the first run is worth keeping anyway, because it shows what the old argument was replaced by rather than whether it helped.** Sample 4 weighed the charge and declined: *"Should I spend $0.05 to research a candidate? AMD, GOOGL, SOFI might be worth a look given the 'Risk-on' environment and strong overall market. However, since I have established profitable positions, and the prompt requests action first, I prioritize the existing positions."* The reason it gives is priority, not cost — **no sample said a bad choice of study would be a loss**, which is the sentence that went. Sample 7, which did order one, still framed it as a budget: *"I will allocate a small amount of my research budget ($0.05)."* **A caution that reads "Should I spend $0.05" is the behaviour this change was aimed at, and it survived on at least one sample.** See the 2026-09-22 entry in JOURNEY.md.
