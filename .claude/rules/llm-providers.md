...
**Every Google request fails after `llm_gemini.REQUEST_TIMEOUT_SECONDS` (300) since 2026-09-24.** Google's SDK has no timeout by default. On 2026-09-24 one decide call never returned, and the pass held `_pass_lock` for ten hours, so every pass after it was skipped. `llm_gemini.client()` puts the timeout into any `http_options` a caller sets, the probe's retry options included. `analysis._build_graph` gives the same value to a Google graph, which covers every analysis and the decision pass's text fallback. `TRADINGAGENTS_LLM_TIMEOUT` (seconds) overrides both. **Other providers keep no timeout on purpose**, because the local pool's proxy queues a request for up to ten minutes. The SDK retries a timeout like a `503`, so one hung request can cost up to five attempts before the pass falls back. That is a bound, not a ration: do not shorten it to save time. `backend/tests/test_gemini_requests_time_out.py` pins it.
