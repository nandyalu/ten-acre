...
`trading-experiment` is the real deployment: one agent, one book, one settings record. A candidate test is not real trading, so it must never touch that container or its state.
...
- **Do not test a candidate inside or through `trading-experiment`.** Not `docker exec`, not the settings page, not the deployed database. See "Keep it off the live container" above.
