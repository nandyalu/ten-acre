...
- **A position-size cap, deliberately absent.** A cap changes what the agent may decide. It needs its own journal entry and reasoning, not a quiet fix. See the 2026-08-29 entry in [the analyst experiment](docs/analyst-experiment.md). The sizing rule in the prompt since 2026-09-19 is a method, not a cap: it says to size from the stop distance and the loss accepted, and Python still refuses nothing on size.
