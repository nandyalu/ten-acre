...
- **The decision-making agent sees far less of a report than the model that wrote it.** `analysis_reader._MAX_CHARS` caps a read at 1,400 characters — "enough for the Rating and the Executive Summary" — out of a stored report that runs to roughly 23,000. The four analyst sections (`market_report`, `sentiment_report`, `news_report`, `fundamentals_report` — MACD and the rest of the technical read, sentiment, news, fundamentals) are computed for every analysis and never reach the agent at all, on a read or in the ambient signals table. The agent is asked to trade real money on a Rating it cannot independently check against the evidence behind it — from a report generated by (possibly) the same model, with none of this agent's own cash, holdings or circumstances in view when it was written. Showing a short summary of each section, not only the executive summary, is the fix; it needs its own probe-the-prompt pass to check the model actually reads the added sections rather than skimming past them the way the change-notes section once did.
